...
let cryptoJsMod = null;
function realCryptoJs() {
  if (!cryptoJsMod) {
    try { cryptoJsMod = require('crypto-js'); } catch { cryptoJsMod = require('./cryptojs-lite'); }
  }
  return cryptoJsMod;
}
// Some providers dereference crypto-js at module top level (e.g. via an
// esbuild `__toESM` helper checking `.__esModule`), so this can't defer a
// real `require()` failure the way the cheerio shim does — it falls back to
// cryptojs-lite.js instead of throwing, covering MD5/HmacMD5/Base64/Utf8/Hex.
const cryptoJsShim = new Proxy({}, { get: (_t, prop) => realCryptoJs()[prop] });

const REQUIRES = {
  cheerio: cheerioShim,
  'cheerio-without-node-native': cheerioShim,
  'react-native-cheerio': cheerioShim,
  'crypto-js': cryptoJsShim,
  crypto: nodeCrypto,
  url: require('url'),
};
